...
const HTML = props => /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, props.children);
exports.HTML = HTML;
HTML.Div = props => {
...
const arrayReverseObj = obj => Object.keys(obj).reverse().map(key => ({
  ...obj[key]
}));
const percentageToGridColumns = (field_sizes, options) => {
  ...
  arr['percent'] = Object.entries(arr['percent']).sort(([, a], [, b]) => a - b).reduce((r, [k, v]) => ({
    ...r,
    [k]: v
  }), {});
  ...
  if (cells_left > 0) {
    ...
    if (cells_left > 0) {
      arr['temp'] = Object.entries(arr['temp']).sort(([, a], [, b]) => a - b).reduce((r, [k, v]) => ({
        ...r,
        [k]: v
      }), {});
    ...
    for (let k2 in rows[k]) {
      ...
      for (let k3 in rows[k][k2]) {
        ...
        if (flag_first_field) {
          if ((rows[k][k2][k3]['label'] ?? '') !== '') {
            ...
            if (label.search('<label') == -1) {
              rows[k][k2][k3]['label'] = HTML.Label({
                value: rows[k][k2][k3]['label'],
                return_html: true
              });
            }
        ...
        if (rows[k][k2][k3]['description']) {
          result += HTML.Text({
            type: 'muted',
            value: rows[k][k2][k3]['description'],
            return_html: true
          });
        }
        result += '</div>';
...
const Tag = props => {
  ...
  let result = /*#__PURE__*/(0, _react.createElement)(tag, options, options.children);
  if (as_html) {
